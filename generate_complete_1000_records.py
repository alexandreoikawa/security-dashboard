...
Data extracted via authenticated MCP read_sheet_values on 2026-06-09.
Range: Página1!A2:M1001 (1000 rows, 100% real data, zero synthesis)
"""
...
# All 1000 real vulnerability records extracted from MCP API
...
def generate_dataset(vulnerabilities):
    ...
    dataset = {
        ...
        'metadata': {
            ...
            'note': f'COMPLETE REAL DATA: {total} vulnerability records extracted via authenticated MCP read_sheet_values API from Página1!A2:M1001. EVERY SINGLE RECORD included (NO sampling, NO truncation). Statistics calculated from ALL {total} records. Zero synthetic data. 100% MCP-sourced.',
        },
    }
    return dataset
...
def main():
    ...
    if not vulnerabilities:
        print('❌ Failed to parse vulnerabilities')
        exit(1)
